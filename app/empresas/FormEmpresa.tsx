...
import { cadastrarEmpresa } from "../actions-negocios";
...
export function FormEmpresa() {
  ...
  function onSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      const res = await cadastrarEmpresa(formData);
      if (res.error) {
        setErro(res.error);
      } else {
        setAberto(false);
  ...
  return (
    <form action={onSubmit} className="mb-5 flex flex-col gap-3 rounded-xl border border-graphite/10 bg-white p-4">
      <input name="nome" required placeholder="Nome da empresa" className="border-0 border-b border-graphite/20 bg-transparent py-2 text-sm outline-none focus:border-gold" />
      <input name="categoria" required placeholder="Ramo de atuação" className="border-0 border-b border-graphite/20 bg-transparent py-2 text-sm outline-none focus:border-gold" />
      ...
      <input name="site" placeholder="Site (opcional)" className="border-0 border-b border-graphite/20 bg-transparent py-2 text-sm outline-none focus:border-gold" />
